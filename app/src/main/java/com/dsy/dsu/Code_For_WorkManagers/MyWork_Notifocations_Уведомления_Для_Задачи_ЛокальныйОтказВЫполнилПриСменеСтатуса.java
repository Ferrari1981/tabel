...
public class MyWork_Notifocations_Уведомления_Для_Задачи_ЛокальныйОтказВЫполнилПриСменеСтатуса extends Worker {
    private  Context context;
    private  String ИмяСлужбыУведомленияДля_ЗадачиТолькоПриСменеСтатусаОтказИлиВыполено = "WorkManager NOtofocationForTasksOnlyDissenOrCompliete";
    ...
    private   Boolean РезультатНужноЗапускатьУведомленияТолькоДляЗадачиОтменыИлиВыполения =false;
    private String PROCESS_ID_УведомленияПлановаяТокькоПриСменеСтатусаОтказИлИВыболненый ="23";
    private   PUBLIC_CONTENT Class_Engine_SQLГдеНаходитьсяМенеджерПотоков =null;
    private  ArrayList БуферСамиУведомленияЛинкСамиУведомления;
    private LinkedBlockingQueue<Long>  БуферСамиУведомленияЛинкСамиУведомленияДляСменыСтатусаУжеПоказывали;
    ...
    // TODO: 28.09.2022
    public MyWork_Notifocations_Уведомления_Для_Задачи_ЛокальныйОтказВЫполнилПриСменеСтатуса(@NonNull Context context, @NonNull WorkerParameters workerParamsвнутри) {
        ...
        messagingStyleДля_ОбщихУведомлений = new NotificationCompat.MessagingStyle(getApplicationContext().getResources().getString(R.string.action_settings)).setConversationTitle("Задача сменила статус");
                ...
                for (ActivityManager.AppTask ТекущаяАктивти : КоличествоЗапущенныйПроуессыДляЧата) {
                    ...
                    if (АктивностьЕслиЕстьTOPДляЧата!=null) {
                        switch (АктивностьЕслиЕстьTOPДляЧата) {
                            // TODO: 24.06.2022                  case "com.dsy.dsu.Code_For_Tasks_КодДля_Задания.MainActivity_Tasks":
                            ...
                            default:
                                ...
                                МетодЗапускаСлужбыУведомленияДлязадачиТОлькоСменаСтатусаОткаВыполенено();
                                ///////todo код запуска уведомлений для чата
                                Log.i(context.getClass().getName(), "Метод ВНУТРИ РАБОТА... С АКТИВТИ ДЕЙСТВУЩИМ ЧАТА ОТРАБОТАЛ ВНУТРИ метода ЗАПУСКАЕМ БЕЗ activity      " +
                                        "   public Result doWork()  MyWork_Notifocations_Уведомления  внутри WORK MANAGER "
                                        + new Date() +
            ...
            // TODO: 25.02.2022 send datas
             myDataОтветОбщегоУведомления = new Data.Builder()
                    .putBoolean("ОтветПослеВыполения_MyWork_Notifocations_Уведомления_Общая",
                            РезультатНужноЗапускатьУведомленияТолькоДляЗадачиОтменыИлиВыполения)
                    .build();
            Log.i(context.getClass().getName(), "ОбщееКоличествоНЕпрочитанныхСтрок"
                    + ОбщееКоличествоНЕпрочитанныхСтрок + " РезультатНужноЗапускатьУведомленияИлиНет "+ РезультатНужноЗапускатьУведомленияТолькоДляЗадачиОтменыИлиВыполения);
        } catch (Exception e) {
            e.printStackTrace();
   ...
   if (РезультатНужноЗапускатьУведомленияТолькоДляЗадачиОтменыИлиВыполения ==true) {
    ...
    private void МетодЗапускаетУведомленияКогдаВообщенетНиОдногоАктивтиNULL(Boolean ФинальныйФлагЛюбогоЗапущеногоАктивти) {
        try{
                          ...
                          ///////todo код запуска уведомлений для чата
                          МетодЗапускаСлужбыУведомленияДлязадачиТОлькоСменаСтатусаОткаВыполенено();
                          ///////todo код апуска уведомлений для чата
    ...
    private void МетодЗапускаСлужбыУведомленияДлязадачиТОлькоСменаСтатусаОткаВыполенено() {
        ...
        try{
            Log.i(getApplicationContext().getClass().getName(), "Запуск метода МетодЗапускаСлужбыУведомления СЛУЖБА СЛУЖБАService_Notifications ДЛЯ ЧАТА ДЛЯ ЧАТА  "+new Date());
                    ...
                    МетодЗарускаСозданиеУведомленийТолькоДляЗадачиСменыСтатсусаОтменаиВыполениея();
    ...
    private void МетодЗарускаСозданиеУведомленийТолькоДляЗадачиСменыСтатсусаОтменаиВыполениея() {
        ...
        try{
            ...
            try{
                ...
                РезультатНужноЗапускатьУведомленияТолькоДляЗадачиОтменыИлиВыполения =false;
                ...
                РезультатНужноЗапускатьУведомленияТолькоДляЗадачиОтменыИлиВыполения =МетодВычисляемСтоитСоздаватьИЗапускатьСлужбуНапоминаний();
                ...
                Log.d(getApplicationContext().getClass().getName(), " Определили Результат НужноЗапускать Уведомления Или Нет  СЛУЖБА"
                        + "--" + РезультатНужноЗапускатьУведомленияТолькоДляЗадачиОтменыИлиВыполения +  " ФлагПолучаемИзНутриПрограммы " +ФлагПолучаемИзНутриПрограммы+ "\n"
...
// TODO: 17.11.2021  ЕСЛИ TRUE ТО НАЧИНАЕМ ЗАПУСКАЕМ УВЕДОМЛЕНИЯ
                ...
                if (РезультатНужноЗапускатьУведомленияТолькоДляЗадачиОтменыИлиВыполения ==true ) {
                    ...
                    МетодКоторыйЗапускаетУвеломленияПослеАнализа(ИнтентДляЗапускаСлужбыПолсеАнализа, РезультатНужноЗапускатьУведомленияТолькоДляЗадачиОтменыИлиВыполения, ФлагПолучаемИзНутриПрограммы);//  //ФлагКтоЗапустилСлужбу

                    Log.d(this.getClass().getName(), "ЗАПУСК ПОСЛЕ АНАЛИЗА ДАТ ЗАПУСКАЕМ УВЕДОМЛЕНИЯ  СЛУЖБА  Синхронизация   " + " ВРЕМЯ " + new Date()
                            + "\n" + " РезультатНужноЗапускатьУведомленияИлиНет " + РезультатНужноЗапускатьУведомленияТолькоДляЗадачиОтменыИлиВыполения);
                ...
                notificationManager.cancel(Integer.parseInt(PROCESS_ID_УведомленияПлановаяТокькоПриСменеСтатусаОтказИлИВыболненый));
            }
 ...
 /*       //////TODO МЕТОД КОТОРЫЙ ЗАПУСКАЕТ УВЕДОМЛЕНИЯ ПОСЛЕ АНАЛИЗА ДАТ
    ...
    private void МетодКоторыйЗапускаетУвеломленияПослеАнализа(Intent intent, boolean результатНужноЗапускатьУведомленияИлиНет, String ФлагКтоЗапустилСлужбу) {
        ...
        Log.d(getApplicationContext().getClass().getName(), " ФлагКтоЗапустилСлужбу " + ФлагКтоЗапустилСлужбу);
            ...
            МетодНотификайшенДЛяОбщейСлужбыУведомленияЗАДАЧАТолькоСменаСтатусаОтменаИлИВполена(ФлагКтоЗапустилСлужбу);
...
/////////TODO запуск нновую нотификашенс устанолвка
    ...
    private void МетодНотификайшенДЛяОбщейСлужбыУведомленияЗАДАЧАТолькоСменаСтатусаОтменаИлИВполена(String ФлагКтоЗапустилСлужбу) {
        try {
            ...
            PendingIntent ЗапускПриКликеКодаПоУведомелениюперерхомПриСтменеСтатусаЗадачи= subClassStartingTasksЗапускДЛяЗадач.
                    МетодПриКликеЗапускаСамогоЗаданияТолькоПриСменеСтатуса(PROCESS_ID_УведомленияПлановаяТокькоПриСменеСтатусаОтказИлИВыболненый,
                            ИмяСлужбыУведомленияДля_ЗадачиТолькоПриСменеСтатусаОтказИлиВыполено,
                            0, "", "ИзУведомленияЗадачаПереходимВАктивтиЗадача");


            ///////TODO запускаем смены стануса задачи черезе PendingIntent
            Log.d(getApplicationContext().getClass().getName(), "PROCESS_ID_УведомленияПлановая  ПЕРЕЙТИ " + PROCESS_ID_УведомленияПлановаяТокькоПриСменеСтатусаОтказИлИВыболненый +
                    " ИмяСлужбыУведомленияДляЧата " + ИмяСлужбыУведомленияДля_ЗадачиТолькоПриСменеСтатусаОтказИлиВыполено + " person.build().getUri() " + person.build().getUri());
      ...
      /*      for(StatusBarNotification statusBarNotification1: statusBarNotificationУведомленияЧата) {
                ...
                if (statusBarNotification1.getId() == Integer.parseInt(PROCESS_ID_УведомленияПлановаяТокькоПриСменеСтатусаОтказИлИВыболненый)) {
         ...
         /*           // TODO: 03.02.2022  УВЕДОМЛЕНИЯ ДЛЯ ОБШЕЙ СИНХРОНИАЗЦИИ
               ...
               /*     NotificationCompat.MessagingStyle.Message messagingStyle2 =
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        ...
                        builder_Для_Задачи = new NotificationCompat.Builder(getApplicationContext(), PROCESS_ID_УведомленияПлановаяТокькоПриСменеСтатусаОтказИлИВыболненый)
                                /////
                                ...
                                .setStyle(messagingStyleДля_ОбщихУведомлений).setColor(Color.parseColor(("#B2222" + new Random().nextInt(1))))
                              ...
                              /*  .addAction(android.R.drawable.ic_btn_speak_now, "Выполнить", ЗапускКОдаЧтоПОльзовательВыполнить)
                                ...
                                .setContentIntent(ЗапускПриКликеКодаПоУведомелениюперерхомПриСтменеСтатусаЗадачи);
                        ...
                        builder_Для_Задачи =
                                new NotificationCompat.Builder(getApplicationContext(), PROCESS_ID_УведомленияПлановаяТокькоПриСменеСтатусаОтказИлИВыболненый)
                                        //
                                        .setContentText(БуферСамиУведомленияЛинкСамиУведомления.toString())                 // .setContentText("http://developer.alexanderklimov.ru/android/")
                                        .setSmallIcon(R.drawable.ic_notifications_black_24dp)////builder.setSmallIcon(R.drawable.ic_launcher_background);//R.mipmap.ic_launcher   ///R.drawable.ic_notifications_black_24dp
                                        .setPriority(NotificationCompat.PRIORITY_MAX)
                                      ...
                                      /*  .addAction(android.R.drawable.ic_btn_speak_now, "Выполнить", ЗапускКОдаЧтоПОльзовательВыполнить)
                                        ...
                                        .setAutoCancel(false)
                                        .setWhen(System.currentTimeMillis()) // автоматически закрыть уведомление после нажатия////.setStyle(new NotificationCompat.BigTextStyle().bigText(bigText) ).setBadgeIconType(NotificationCompat.BADGE_ICON_SMALL)
                                        .setContentIntent(ЗапускПриКликеКодаПоУведомелениюперерхомПриСтменеСтатусаЗадачи);// TODO: 27.03.2022    .addAction(android.R.drawable.ic_delete, "Выполнил/на", ЗапускКОдаЧтоПОльзовательОзнаомленсЗаданиемss)
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                NotificationChannel channel = new NotificationChannel(
                        PROCESS_ID_УведомленияПлановаяТокькоПриСменеСтатусаОтказИлИВыболненый,
                        "Channel human readable title",
                        NotificationManager.IMPORTANCE_HIGH);
                mNotificationManagerДляЧАТА.createNotificationChannel(channel);
                builder_Для_Задачи.setChannelId(String.valueOf(PROCESS_ID_УведомленияПлановаяТокькоПриСменеСтатусаОтказИлИВыболненый));
                ...
                mNotificationManagerДляЧАТА.notify(Integer.parseInt(PROCESS_ID_УведомленияПлановаяТокькоПриСменеСтатусаОтказИлИВыболненый), builder_Для_Задачи.build());////   mNotificationManagerДляЧАТА.notify(Integer.parseInt(PROCESS_ID_УведомленияПлановая), builder.build());
...
// TODO: 20.06.2022  метод смены статуса уведосления что его показывали
                    ...
                    МетодСтатусаЗадачиЧтоУведомлениеУжеПоказывалиВуведомлениеТолькоОтказИлиВыполнить(БуферСамиУведомленияЛинкСамиУведомленияДляСменыСтатусаУжеПоказывали);
    ...
    private void МетодСтатусаЗадачиЧтоУведомлениеУжеПоказывалиВуведомлениеТолькоОтказИлиВыполнить( @NonNull  LinkedBlockingQueue  БуферСамиУведомленияЛинкСамиУведомленияДляСменыСтатусаУжеПоказывали ) throws ExecutionException, InterruptedException {
        ...
        try{
            ...
            Log.d(this.getClass().getName(), " БуферСамиУведомленияЛинкСамиУведомленияДляСменыСтатусаУжеПоказывали  "
                    + БуферСамиУведомленияЛинкСамиУведомленияДляСменыСтатусаУжеПоказывали);

            БуферСамиУведомленияЛинкСамиУведомленияДляСменыСтатусаУжеПоказывали.forEach(new Consumer() {
                @Override
                public void accept(Object UUIDРочитаногоЗаданиеДляКотрогоДалееБудетПроизведенаСменаСтсусаНАОзнакомленный) {

                    Log.d(this.getClass().getName(), " UUIDРочитаногоЗаданиеДляКотрогоДалееБудетПроизведенаСменаСтсусаНАОзнакомленный  "
                            + UUIDРочитаногоЗаданиеДляКотрогоДалееБудетПроизведенаСменаСтсусаНАОзнакомленный);
                    
                    try{

                        // TODO: 24.06.2022 start
                        ...
                        ContentValues contentValuesСменаСтатусаЗадачиЧтоЕЕЕжуПоказывалиТолькоДЛяУведолменияотказИлилВЫполнилСменыСтатуса = new ContentValues();
                        ///
                        contentValuesСменаСтатусаЗадачиЧтоЕЕЕжуПоказывалиТолькоДЛяУведолменияотказИлилВЫполнилСменыСтатуса.put("alreadyshownnotifications", 4);
                        ...
                        ///
                        contentValuesСменаСтатусаЗадачиЧтоЕЕЕжуПоказывалиТолькоДЛяУведолменияотказИлилВЫполнилСменыСтатуса.put("current_table", РезультатУвеличинаяВерсияВнутриСамогоТабелСтрудникаПервая);
                        ...
                        ///
                        class_grud_sql_operationsПослеУдаленияДобавляемДатуВерсии.concurrentHashMapНаборПараментовSQLBuilder_Для_GRUD_Операций
                                .put("ЗначениеФлагОбновления",UUIDРочитаногоЗаданиеДляКотрогоДалееБудетПроизведенаСменаСтсусаНАОзнакомленный.toString());
                        ///
                        ...
                        class_grud_sql_operationsПослеУдаленияДобавляемДатуВерсии.contentValuesДляSQLBuilder_Для_GRUD_Операций.putAll(contentValuesСменаСтатусаЗадачиЧтоЕЕЕжуПоказывалиТолькоДЛяУведолменияотказИлилВЫполнилСменыСтатуса);
                        ...
                        // TODO: 24.06.2022 end
                    БуферСамиУведомленияЛинкСамиУведомленияДляСменыСтатусаУжеПоказывали.peek();
                        // TODO: 24.06.2022
                } catch (Exception e) {
                    e.printStackTrace();
                    ///метод запись ошибок в таблицу
...
//
    ...
    private boolean МетодВычисляемСтоитСоздаватьИЗапускатьСлужбуНапоминаний() {
        ...
        try {
            ...
            //
            class_grud_sql_operationsIDпользоввателяДляСлужб. concurrentHashMapНаборПараментовSQLBuilder_Для_GRUD_Операций.put("ФорматПосика"," status_write<> ? AND alreadyshownnotifications<>? " +
            ...
            //
            class_grud_sql_operationsIDпользоввателяДляСлужб. concurrentHashMapНаборПараментовSQLBuilder_Для_GRUD_Операций.put("УсловиеПоиска2",4);
          ...
          //  class_grud_sql_operationsIDпользоввателяДляСлужб. concurrentHashMapНаборПараментовSQLBuilder_Для_GRUD_Операций.put("УсловиеЛимита","1");
            // TODO: 27.08.2021  ПОЛУЧЕНИЕ ДАННЫХ ОТ КЛАССА GRUD-ОПЕРАЦИИ
            ///
            SQLiteCursor   Курсор_ДляСлужбыУведомлений_ТолькоДляЗадачаТолькоДляОтказИлиДополниетельно =null;


            Курсор_ДляСлужбыУведомлений_ТолькоДляЗадачаТолькоДляОтказИлиДополниетельно= (SQLiteCursor)  class_grud_sql_operationsIDпользоввателяДляСлужб.
            ...
            Log.d(this.getClass().getName(), "Курсор_ДляСлужбыУведомлений_ТолькоДляЧата "  +Курсор_ДляСлужбыУведомлений_ТолькоДляЗадачаТолькоДляОтказИлиДополниетельно);
...
*/
            ...
            if ( Курсор_ДляСлужбыУведомлений_ТолькоДляЗадачаТолькоДляОтказИлиДополниетельно.getCount()>0) {
                //////


                РезультатНужноЗапускатьУведомленияТолькоДляЗадачиОтменыИлиВыполения =true;

                ОбщееКоличествоНЕпрочитанныхСтрок=       Курсор_ДляСлужбыУведомлений_ТолькоДляЗадачаТолькоДляОтказИлиДополниетельно.getCount();
                ...
                МетодПолучениеДатыПослеПолучениеПравСотрудникаДля_ОбщихУведомления(Курсор_ДляСлужбыУведомлений_ТолькоДляЗадачаТолькоДляОтказИлиДополниетельно);
                ...
                Log.w(this.getClass().getName(), "  СЛУЖБА...  НЕТ СООБЩЕНИ ДЛЯ ОТОБРАЖЕНИЯ В ЧАТЕ НЕТ ПРОПУСКАЕМ ХОЛОСТОЙ ХОД СЛУЖБЫ УВЕДОМЛЕНИЯ ЧАТ " +ПолучениеПравДляТаблицыПрава+
                         " Курсор_ДляСлужбыУведомлений_ТолькоДляЗадачаТолькоДляОтказИлиДополниетельно " +Курсор_ДляСлужбыУведомлений_ТолькоДляЗадачаТолькоДляОтказИлиДополниетельно);
        ...
        return РезультатНужноЗапускатьУведомленияТолькоДляЗадачиОтменыИлиВыполения;
    }
    ...
    private void МетодПолучениеДатыПослеПолучениеПравСотрудникаДля_ОбщихУведомления( @NonNull  SQLiteCursor  Курсор_ДляСлужбыУведомлений_ТолькоДляЧата) throws ParseException {
        ...
        try{
            ...
            if (Курсор_ДляСлужбыУведомлений_ТолькоДляЧата.getCount()>0) {
                // TODO: 28.06.2022  
                if (!Курсор_ДляСлужбыУведомлений_ТолькоДляЧата.isFirst()) {
                    // TODO: 28.06.2022
            ...
            БуферСамиУведомленияЛинкСамиУведомленияДляСменыСтатусаУжеПоказывали=new LinkedBlockingQueue<>();
    ...
    /*            class_grud_sql_operationsIDпользоввателяДляСлужб. concurrentHashMapНаборПараментовSQLBuilder_Для_GRUD_Операций.put("УсловиеСортировки","date_update DESC");
      ...
      /*          // TODO: 03.02.2022
               ...
               Integer СамСтатусПрочтенияПриВыданнйоЗадании= Курсор_ДляСлужбыУведомлений_ТолькоДляЧата.getInt(ИндексСтатусПрочтенияПриВыданнйоЗадании);
                ...
                // TODO: 15.11.2021
                Integer ИндексУжеUUDиУведомленияЗадании=   Курсор_ДляСлужбыУведомлений_ТолькоДляЧата.getColumnIndex("uuid");
                /////
                Long UUIDзадачиУжеПоказывалиТекущееУведомления = Курсор_ДляСлужбыУведомлений_ТолькоДляЧата.getLong(ИндексУжеUUDиУведомленияЗадании);


                Log.d(this.getClass().getName(), " UUIDзадачиУжеПоказывалиТекущееУведомления  " + UUIDзадачиУжеПоказывалиТекущееУведомления);
...
// TODO: 20.06.2022  заполенияе тольк если данное уведомления задачиеще не показывали
                if (СтатусзадачиУжеПоказывалиТекущееУведомления!=4  && СтатусзадачиТекущееУведомленияСтатус>0  ) {
                    // TODO: 20.06.2022  
                    БуферСамиУведомленияЛинкСамиУведомления.add("заголовок:"+" "+СамСтатусIDЗадание+"\n"+
                            ...
                            "смена статуса на: ("+СтатусПоказывмПользователю+")"+"\n");
                    // TODO: 24.06.2022  для смены статуса заполянем еше дну очтрез с UUID  сообещний

                    БуферСамиУведомленияЛинкСамиУведомленияДляСменыСтатусаУжеПоказывали.offer(UUIDзадачиУжеПоказывалиТекущееУведомления);
    ...
    private void МетодПослеЗапоелнияВЦиклеЗадачЗаполянем(String КтоНаписалСообщениеФИО,
        ...
        // person.setIcon( IconCompat.createWithResource(Контекст, R.drawable.icon_dsu1_for_fragment1_chat2));
            ...
            // TODO: 03.02.2022  УВЕДОМЛЕНИЯ ДЛЯ ОБШЕЙ СИНХРОНИАЗЦИИ
            messagingStyleДля_ОбщихУведомлений.addMessage(person.build().getKey(), System.currentTimeMillis(), "создано:" + person.build().getName()
                    + " +" +
