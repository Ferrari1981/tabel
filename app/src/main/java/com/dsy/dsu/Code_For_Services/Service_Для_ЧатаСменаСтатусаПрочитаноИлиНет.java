        ...
        try {
         ...
         modelДляФрагментаДляОперацииЗаписиНовгоСтатусаПрочитанного = new Class_MODEL_synchronized(context);
            ...
            РезультатУвеличинаяВерсияДАныхЧата =
                    class_grud_sql_operations.new ChangesVesionData(context).
                            МетодПовышаемВерсииCurrentTable
                                    (ТаблицаОбработкиВнутриЧтатаПриУвеличсенииВерсииДаннвъКоглаПрочинаноСообещния, context, Create_Database_СсылкаНАБазовыйКласс.getССылкаНаСозданнуюБазу());
...
,@NonNull Integer IDСкемПереписываюсь)
   ...
   try{
       ...
       Long   ЛокльныйUUIDТОлькоДЛЯОднойТаблицыChats =
                    (Long) new Class_Generation_UUID(context).МетодГенерацииUUID(context);
            ...
            Long    РезультатУвеличинаяВерсияДАныхЧата = class_grud_sql_operations.new ChangesVesionData(context).
                    МетодПовышаемВерсииCurrentTable(ПерваяТаблицыОбработкиТаблицаЧат,  context, Create_Database_СсылкаНАБазовыйКласс.getССылкаНаСозданнуюБазу());
       // TODO: 18.11.2022
    ...
    ,@NonNull Context context) {
        ...
        try {
            ...
            if (НовыйUUIDДляОбеихТаблицЧАТиДАТАЧАТдляПоляPARENT_UUID > 0) {
                ...
                Long ЛокальныйUUIDДляТаблицыДатаЧатВтораяТаблица = (Long) new Class_Generation_UUID(context).МетодГенерацииUUID(context);
                ...
                Long      РезультатУвеличинаяВерсияДАныхДатЧата =
                        class_grud_sql_operations.new ChangesVesionData(context).
                                МетодПовышаемВерсииCurrentTable
                                        (ТаблицаВторойОбработкиДляТаблицыДата_Табеля,  context, Create_Database_СсылкаНАБазовыйКласс.getССылкаНаСозданнуюБазу());
                // TODO: 18.11.2022
